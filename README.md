...
- 支持在软件内使用 `/bbind` 临时绑定码绑定角色，并提供无账号引导和官网备用入口。
- 读取当前登录账号已经绑定的全部游戏账号，支持账号切换、继续绑定和带确认的解绑。
...
- “对局数据”面板可按模式和日期统计本人击败、伤害、资源、物品、升级及常用队友。
- 缺失对局详情支持自适应批量补全；已补全详情持久保存在本地，后续统计不再请求服务器。
- 提供每日胜负频次图和累计连胜折线图。
...
- 战绩抓取使用顺序自适应均衡策略，初始请求间隔默认 `0.1` 秒，可按 `0.1` 秒精度调整。
- 服务器返回“请勿频繁请求”或 HTTP 429 时，程序会自动延长等待时间并重试当前页。
- 详情补全默认使用自适应均衡模式，从低并发开始并在稳定时逐步提速；遇到限流会自动暂停、降速和重试。
- 详情补全支持取消和断点继续，已完成详情会批量写入本地缓存。
...
- 临时绑定码仅用于一次绑定请求，不写入本地缓存；解绑不会删除该 UUID 的本地玩家资料和战绩缓存。
